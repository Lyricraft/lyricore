...
public class ServerResponseManager extends AbstractResponseManager<ClientRequestPair> {
    ...
    protected void handleRequest(CompoundTag bodyNbt, IPayloadContext context){
        ...
        ClientRequestPair pair = pairs.get(metaNbt.getString("type"));
        if (pair == null) {
            ...
            if (strict)
                context.disconnect(Component.translatable("lyricore.multiplayer.disconnect.invalid_request")
                        .append("\n" + name.toString() + " . " + metaNbt.getString("type")));
            return;
        }
    ...
    public class Handle extends AbstractResponseManager.Handle{
        ...
        protected void send(CompoundTag nbt) {
            PacketDistributor.sendToPlayer((ServerPlayer) (context.player()), new ManagedRequestPayload(nbt));
        }
    }
